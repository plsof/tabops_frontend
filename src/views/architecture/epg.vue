...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchEpg(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createEpg(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateEpg(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'Epg', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteEpg(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchEpg(downloadlistQuery).then(response => {
