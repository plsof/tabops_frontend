...
import { fetchMGS, createMGS, updateMGS, deleteMGS } from '@/api/architecture/mgs'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchMGS(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('mgs-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createMGS(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateMGS(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'MGS', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteMGS(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchMGS(downloadlistQuery).then(response => {
