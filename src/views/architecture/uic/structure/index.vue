...
import { fetchUic, createUic, updateUic, deleteUic } from '@/api/architecture/uic'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchUic(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('uic-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createUic(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateUic(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'Uic', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteUic(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchUic(downloadlistQuery).then(response => {
