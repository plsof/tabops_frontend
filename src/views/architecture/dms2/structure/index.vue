...
import { fetchDms2, createDms2, updateDms2, deleteDms2 } from '@/api/architecture/dms2'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchDms2(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('dms2-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createDms2(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateDms2(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'DMS2', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteDms2(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchDms2(downloadlistQuery).then(response => {
