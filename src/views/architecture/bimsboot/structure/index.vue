...
import { fetchBimsBoot, createBimsBoot, updateBimsBoot, deleteBimsBoot } from '@/api/architecture/bimsboot'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchBimsBoot(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('bimsboot-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createBimsBoot(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateBimsBoot(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'BImsBoot', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteBimsBoot(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchBimsBoot(downloadlistQuery).then(response => {
