...
import { fetchTms, createTms, updateTms, deleteTms } from '@/api/architecture/tms'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchTms(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('tms-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createTms(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateTms(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'Tms', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteTms(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchTms(downloadlistQuery).then(response => {
