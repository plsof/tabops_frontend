...
import { fetchXMpp, createXMpp, updateXMpp, deleteXMpp } from '@/api/architecture/xmpp'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchXMpp(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('xmpp-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createXMpp(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateXMpp(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'XMpp', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteXMpp(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchXMpp(downloadlistQuery).then(response => {
