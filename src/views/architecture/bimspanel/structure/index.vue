...
import { fetchBimsPanel, createBimsPanel, updateBimsPanel, deleteBimsPanel } from '@/api/architecture/bimspanel'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchBimsPanel(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('bimspanel-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createBimsPanel(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateBimsPanel(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'BImsPanel', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteBimsPanel(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchBimsPanel(downloadlistQuery).then(response => {
