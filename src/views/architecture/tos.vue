...
import { fetchTOS, createTOS, updateTOS, deleteTOS } from '@/api/architecture/tos'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchTOS(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('tos-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createTOS(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateTOS(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'TOS', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteTOS(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchTOS(downloadlistQuery).then(response => {
