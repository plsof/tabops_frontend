...
import { fetchUCS, createUCS, updateUCS, deleteUCS } from '@/api/architecture/ucs'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchUCS(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('ucs-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createUCS(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateUCS(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'UCS', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteUCS(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchUCS(downloadlistQuery).then(response => {
