...
import { fetchPic, createPic, updatePic, deletePic } from '@/api/architecture/pic'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchPic(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('pic-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createPic(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updatePic(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'Pic', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deletePic(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchPic(downloadlistQuery).then(response => {
