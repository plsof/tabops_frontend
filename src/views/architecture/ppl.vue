...
import { fetchPpl, createPpl, updatePpl, deletePpl } from '@/api/architecture/ppl'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchPpl(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('ppl-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createPpl(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updatePpl(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'Ppl', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deletePpl(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchPpl(downloadlistQuery).then(response => {
