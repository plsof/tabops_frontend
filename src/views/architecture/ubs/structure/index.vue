...
import { fetchUBS, createUBS, updateUBS, deleteUBS } from '@/api/architecture/ubs'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchUBS(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('ubs-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createUBS(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateUBS(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'UBS', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteUBS(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchUBS(downloadlistQuery).then(response => {
