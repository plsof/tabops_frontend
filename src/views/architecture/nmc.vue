...
import { fetchNMC, createNMC, updateNMC, deleteNMC } from '@/api/architecture/nmc'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchNMC(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('nmc-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createNMC(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateNMC(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'NMC', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteNMC(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchNMC(downloadlistQuery).then(response => {
