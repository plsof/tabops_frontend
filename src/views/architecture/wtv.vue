...
import { fetchWtv, createWtv, updateWtv, deleteWtv } from '@/api/architecture/wtv'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchWtv(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('wtv-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createWtv(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateWtv(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'Wtv', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteWtv(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchWtv(downloadlistQuery).then(response => {
