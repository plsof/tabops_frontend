...
import { fetchVAS, createVAS, updateVAS, deleteVAS } from '@/api/architecture/vas'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchVAS(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('vas-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createVAS(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateVAS(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'VAS', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteVAS(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchVAS(downloadlistQuery).then(response => {
