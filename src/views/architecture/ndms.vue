...
import { fetchNDms, createNDms, updateNDms, deleteNDms } from '@/api/architecture/ndms'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchNDms(this.listQuery).then(response => {
    ...
    getService() {
      fetchService().then(response => {
        this.serviceOptions = response.data.filter(element => {
          return element['name'].indexOf('ndms-') === 0
        })
      })
    },
    handleFilter() {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createNDms(this.temp).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateNDms(tempData).then((response) => {
    ...
    handleRefresh(row) {
      ...
      refresh({ db: 'NDms', id: this.temp.id }).then(response => {
        if (response.code === 0) {
    ...
    handleDelete(row) {
      ...
      // 删除后端接口数据
      deleteNDms(row.id).then(response => {
      })
      this.$notify({
        title: 'Success',
    ...
    handleDownload() {
      ...
      fetchNDms(downloadlistQuery).then(response => {
